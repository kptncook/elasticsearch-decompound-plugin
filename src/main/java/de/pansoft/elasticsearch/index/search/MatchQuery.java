...
import static org.elasticsearch.index.search.MatchQuery.Type;
import static org.elasticsearch.index.search.MatchQuery.ZeroTermsQuery;
...
public class MatchQuery {
    ...
    private boolean hasPositions(MappedFieldType fieldType) {
        return fieldType.getTextSearchInfo().hasPositions();
    }
    ...
    class MatchQueryBuilder extends QueryBuilder {
         ...
         */
        protected Query createBooleanPrefixQuery(String field, String queryText, Occur occur) {
            return createQuery(field, queryText, Type.BOOLEAN_PREFIX, occur, 0);
        }

        private Query createFieldQuery(TokenStream source, Type type, Occur operator, String field, int phraseSlop) {
            assert operator == Occur.SHOULD || operator == Occur.MUST;
        ...
        private Query createQuery(String field, String queryText, Type type, Occur operator, int phraseSlop) {
        ...
        private void add(BooleanQuery.Builder q, String field, List<Term> current, Occur operator, boolean isPrefix) {
            if (current.isEmpty()) {
                return;
            }
            if (current.size() == 1) {
        ...
        private Query analyzeMultiBoolean(String field, TokenStream stream,
                                          Occur operator, boolean isPrefix) throws IOException {
                ...
                if (graph.hasSidePath(start)) {
                    ...
                    Iterator<Query> queries = new Iterator<Query>() {
                        ...
                        public Query next() {
                            ...
                            return createFieldQuery(ts, type, Occur.MUST, field, 0);
                        }
                    };
        ...
        private Query analyzeGraphPhrase(TokenStream source, String field, Type type, int slop) throws IOException {
            ...
            if (phraseSlop > 0) {
                ...
                while (it.hasNext()) {
                    Query query = createFieldQuery(it.next(), type, Occur.MUST, field, slop);
                    if (query != null) {
