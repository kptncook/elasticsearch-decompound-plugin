...
public class MatchQueryParser {
    ...
    public enum Type implements Writeable {
        ...
        public static MatchQueryParser.Type readFromStream(StreamInput in) throws IOException {
            int ord = in.readVInt();
            for (MatchQueryParser.Type type : MatchQueryParser.Type.values()) {
                if (type.ordinal == ord) {
                    return type;
                }
            }
    ...
    /**
     * @deprecated See {@link MatchQueryParser.MatchQueryBuilder#setCommonTermsCutoff(Float)} for more details
     */
    @Deprecated
    ...
    public Query parse(MatchQueryParser.Type type, String fieldName, Object value) throws IOException {
        ...
        Analyzer analyzer = getAnalyzer(fieldType, type == MatchQueryParser.Type.PHRASE || type == MatchQueryParser.Type.PHRASE_PREFIX);
        assert analyzer != null;

        MatchQueryParser.MatchQueryBuilder builder = new MatchQueryParser.MatchQueryBuilder(analyzer, fieldType, enablePositionIncrements, autoGenerateSynonymsPhraseQuery);
         ...
         */
        if (analyzer == Lucene.KEYWORD_ANALYZER && type != MatchQueryParser.Type.PHRASE_PREFIX) {
            final Term term = new Term(resolvedFieldName, stringValue);
            if (type == MatchQueryParser.Type.BOOLEAN_PREFIX
    ...
    private Query createCommonTermsQuery(MatchQueryParser.MatchQueryBuilder builder, String field, String queryText,
    ...
    class MatchQueryBuilder extends QueryBuilder {
        ...
        protected Query createFieldQuery(Analyzer analyzer, BooleanClause.Occur operator, String field,
            ...
            MatchQueryParser.Type type = quoted ? MatchQueryParser.Type.PHRASE : MatchQueryParser.Type.BOOLEAN;
        ...
        protected Query createPhrasePrefixQuery(String field, String queryText, int slop) {
            return createQuery(field, queryText, MatchQueryParser.Type.PHRASE_PREFIX, occur, slop);
        }
        ...
        protected Query createBooleanPrefixQuery(String field, String queryText, BooleanClause.Occur occur) {
            return createQuery(field, queryText, MatchQueryParser.Type.BOOLEAN_PREFIX, occur, 0);
        }

        private Query createFieldQuery(TokenStream source, MatchQueryParser.Type type, BooleanClause.Occur operator, String field, int phraseSlop) {
            assert operator == BooleanClause.Occur.SHOULD || operator == BooleanClause.Occur.MUST;
            ...
            try (CachingTokenFilter stream = new CachingTokenFilter(source)) {
                ...
                if (numTokens == 0) {
                    ...
                    // single term
                    if (type == MatchQueryParser.Type.PHRASE_PREFIX) {
                        return analyzePhrasePrefix(field, stream, phraseSlop, positionCount);
                    } else {
                        return analyzeTerm(field, stream, type == MatchQueryParser.Type.BOOLEAN_PREFIX);
                    }
                } else if (isGraph) {
                    // graph
                    if (type == MatchQueryParser.Type.PHRASE || type == MatchQueryParser.Type.PHRASE_PREFIX) {
                        return analyzeGraphPhrase(stream, field, type, phraseSlop);
                    } else {
                        return analyzeGraphBoolean(field, stream, operator, type == MatchQueryParser.Type.BOOLEAN_PREFIX);
                    }
                } else if (type == MatchQueryParser.Type.PHRASE && positionCount > 1) {
                    // phrase
                    if (hasSynonyms) {
                ...
                } else if (type == MatchQueryParser.Type.PHRASE_PREFIX) {
                    // phrase prefix
                    return analyzePhrasePrefix(field, stream, phraseSlop, positionCount);
                } else {
                    // boolean
                    if (positionCount == 1) {
                        ...
                        return analyzeMultiBoolean(field, stream, operator, type == MatchQueryParser.Type.BOOLEAN_PREFIX);
                    }
                }
            } catch (IOException e) {
        ...
        private Query createQuery(String field, String queryText, MatchQueryParser.Type type, BooleanClause.Occur operator, int phraseSlop) {
        ...
        private Query analyzeGraphPhrase(TokenStream source, String field, MatchQueryParser.Type type, int slop) throws IOException {
            assert type == MatchQueryParser.Type.PHRASE_PREFIX || type == MatchQueryParser.Type.PHRASE;
            ...
            for (int i = 0; i <= articulationPoints.length; i++) {
                ...
                boolean usePrefix = end == -1 && type == MatchQueryParser.Type.PHRASE_PREFIX;
                if (graph.hasSidePath(start)) {
